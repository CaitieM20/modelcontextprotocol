...
// The base RequestParams type includes fields for retried requests.
...
Since this change creates a polymorphic response for method calls like 'tools/call', we are introducing a new field to `Result` which indicate the `ResultType` the client should parse. If this field is not provided the Client should assume a `ResultType` of "complete" for backwards compatibility. The schema change will look like this:
...
export interface Result {
  _meta?: MetaObject;
  // New field to indicate the type of the result, which allows the client to determine how to parse the result object. If no result_type is specified "complete" should be assumed.
...
export type ResultType =
  ...
  | "incomplete"; // the request is incomplete and the result contains an {@link IncompleteResult} object
```
...
Note that the ephemeral workflow is incompatible with URL-based
elicitations; those will instead be supported only via the persistent
workflow described below. As a result, there is no longer a need for
the `URLElicitationRequiredError` error code. That code will be removed
from the spec.
...
Note that for URL-based elicitation requests, there will be no input
response sent via the `tasks/input_response` method. Instead, the
elicitation response will be sent via the external URL, and the task
state will automatically change from `input_required` to `working` or
