...
read results. This allows clients to cache responses and reduce unnecessary re-fetching.
...
A `cacheScope` of `"public"` indicates that the response does not contain user-specific data and can be safely shared. However, servers MUST ensure that responses marked as `"public"` can lead to primitives being exposed across users and auth context.
Server implementors should ensure that:
- the `cacheScope` correctly reflects the intended visibility of the primitive.
- Server authors MUST apply appropriate per-primitive access controls, and MUST NOT rely on 
